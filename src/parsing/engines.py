...
class WorkUA(PageQuery):
    ...
    def _prepare_offer(self, raw_offer: Element) -> OfferModel:
        ...
        salary_to = extracted_salary.pop() if extracted_salary else None
        salary_from = extracted_salary.pop() if extracted_salary else None
        ...
        time_publish = raw_offer.find('div.col-sm-push-7.col-sm-5.col-xs-12.add-top > div > span.text-muted.small',
                                      first=True)
        time_publish = time_publish.text if time_publish else None
        time_publish = None if time_publish in ["Гаряча", "Відгук без резюме"] else time_publish

        return OfferModel(
            title=title, city=city.text if city else None, salary_from=salary_from, salary_to=salary_to,
            company=company,
            description=desc, link=link, time_publish=time_publish or None
        )
